/**
 * validate-ds.ts — Script para validar todos los JSONs de DS contra sus schemas Zod
 */
...
const DS_DATA_DIR = join(import.meta.dirname, '..', 'src', 'knowledge', 'ds', 'data');

const DS_FILES: { file: string; area: KBArea }[] = [
  // Compartidas
...
for (const { file, area } of DS_FILES) {
  ...
  try {
    const raw = JSON.parse(readFileSync(join(DS_DATA_DIR, file), 'utf-8'));
